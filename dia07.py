...
datos = open("dia07-input.txt", "r").read().splitlines()

datos_lista = [list(d) for d in datos]

filas = len(datos_lista)
cols = len(datos_lista[0])

contador = 0

for f in range(1,filas):
    for c in range(cols):
        if datos_lista[f-1][c] == "S":
            if datos_lista[f][c] == ".":
                datos_lista[f][c] = "|"
        elif datos_lista[f-1][c] == "|":
            if datos_lista[f][c] == ".":
                datos_lista[f][c] = "|"
            elif datos_lista[f][c] == "^":
                if c > 0 and datos_lista[f][c-1] == ".":
                    datos_lista[f][c-1] = "|"
                if c < cols-1 and datos_lista[f][c+1] == ".":
                    datos_lista[f][c+1] = "|"
                contador += 1
            ...
            if datos_lista[f-1][c] == "S":
                ...
                if datos_lista[f][c] == ".":
                    ...
                    if c==0:
                        ...
                        if aleatorio == 0 and datos_lista[f][c-1] == ".":
                            datos_lista[f][c-1] = "|"
                        elif datos_lista[f][c+1] == ".":
                                datos_lista[f][c+1] = "|"
    if datos_lista not in conjunto_caminos:
        conjunto_caminos.append(datos_lista.copy())
    print(len(conjunto_caminos))
